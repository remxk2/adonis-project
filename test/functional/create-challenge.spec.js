...
test('cannot craete a challenge if not authenticated', async ({ assert, client }) => {
  const data = {
    title: 'Top 5 2018 Movies to watch',
    description: 'A list of 5 movies from 2018 to absolutely watched'
  }
...
test('cannot create challenge if no title', async ({ assert, client }) => {
  const user = await Factory.model('App/Models/User').create()
...
test('cannot create challenge if title and description are not a string', async ({ assert, client }) => {
  const user = await Factory.model('App/Models/User').create()
